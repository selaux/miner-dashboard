    ...
    getNotificationsForMinAvgHashrate: function (moduleId, data) {
        var self = this,
            ...
            notifyOnHashrate = function (minAvgHashrate) {
                if (_.isNumber(data.avgHashrate) && data.avgHashrate <= minAvgHashrate) {
                    if (!_.contains(self.notifiedForHashrate, moduleId)) {
                        notifications.push('Hashrate ' + data.avgHashrate + ' MH/s of ' + moduleId + ' has dropped below ' + minAvgHashrate + ' MH/s');
