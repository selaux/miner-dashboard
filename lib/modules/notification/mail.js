    ...
    getNotificationsForMinAvgHashrate: function (moduleId, data) {
        var self = this,
            ...
            notifyOnHashrate = function (minAvgHashrate) {
                if (_.isNumber(data.currentHashrate) && data.currentHashrate <= minAvgHashrate) {
                    if (!_.contains(self.notifiedForHashrate, moduleId)) {
                        notifications.push('Hashrate ' + data.currentHashrate + ' MH/s of ' + moduleId + ' has dropped below ' + minAvgHashrate + ' MH/s');
